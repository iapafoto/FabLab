...
module porte() {
    translate([-rBase+2,0,hDoor/2+bDoor]) {
        cube([4, wDoor+2*bDoor, hDoor+bDoor], center=true);   
        intersection() {
            rotate(90,[0,1,0]){         
                cylinder(h=4.1, r=hDoor*.62, center = true, $fn=resolution);
            }
            translate([0,0,wDoor/2.]) {
                cube([4, wDoor+2*bDoor, hDoor+4], center=true);
            } 
       }
   } 
}
...
module murs() {
    intersection() {
        difference() {
            ...
            cylinder( h=hMur, r1 = rBase-epWall, r2 = rTop-epWall, center = false, $fn=resolution);
...
module pale(lTige, lPale, wPale, ep) {
    ...
    for ( i = [0:nb] ) {
        translate([lTige + k*i, 0, ep/2]) {
            cube([ep,wPale,ep], center = true);
        }
    }
}
...
module moulin(withPale) {
    ...
    difference() {
        union() {
        // Boiseries porte
            translate([-rBase+3.6,0,(hDoor+bDoor)/2])
    ...
    // Boiseries fenetre
            ...
            translate([-rBase+5,0, altiWin]) {
                 cube([2.5, .3, hWin+.1], center=true);
            }
            translate([-rBase+5,0, altiWin-(hWin/2)+hWin/3]) {
                 cube([2.5, wWin+.1, .3], center=true);
            }
...
module revolve_text(radius, chars) {
    ...
    chars_len = len(chars);
    font_size = 1.5*circumference / chars_len;
    ...
    difference() {
        ...
        translate([0,0,1.7])
            linear_extrude(height = 1.5, center = true, convexity = 10, twist = 0)
                revolve_text(radius*.9, chars);
    }

    translate([0,1.3*rBase,2]) scale([2,2,1.5]) occitane();
//    translate([0,1.3*rBase,2]) rotate(180,[0,0,1]) scale([2.6, 2.6, .6]) logoTelethon();
}


//translate([1.9*rBase+wPale,1.9*rBase+wPale,0]) pales(lTige,lPale,wPale,epPale);
translate([0,-rBase/4,2]) moulin(true);
socle();
